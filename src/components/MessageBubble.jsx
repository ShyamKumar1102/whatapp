...
export default function MessageBubble({ message, onDeleteForMe, onDeleteForEveryone }) {
  ...
  const handleConfirm = () => {
    if (confirmType === 'me') onDeleteForMe?.(message.id);
    if (confirmType === 'everyone') onDeleteForEveryone?.(message.id);
    setConfirmType(null);
  };
  ...
  return (
    <>
        ...
        <div className="relative max-w-[70%]">
          ...
          {showMenu && (
            ...
            >
              ...
              <button
                onClick={() => { setShowMenu(false); setConfirmType('me'); }}
                className="w-full text-left px-4 py-2.5 text-sm text-foreground hover:bg-surface-hover flex items-center gap-3 transition-colors"
              >
              ...
              {isOutgoing && !isDeleted && (
                <button
                  onClick={() => { setShowMenu(false); setConfirmType('everyone'); }}
      ...
      <ConfirmDialog
        ...
        description="This message will be deleted for you only. Others can still see it."
