          ...
          return (
            <div key={template.id} className="bg-card rounded-xl border border-border p-5 hover:shadow-sm transition-shadow">
              ...
              <div className="flex items-center justify-between mt-3">
                <p className="text-[10px] text-muted-foreground">Created {template.createdAt}</p>
                <div className="flex gap-1">
                  <button onClick={() => handleCopy(template.body)} className="p-1.5 rounded hover:bg-muted transition-colors" title="Copy body">
                    <Copy className="w-3.5 h-3.5 text-muted-foreground" />
                  </button>
                  <button onClick={() => handleUseTemplate(template)} className="p-1.5 rounded hover:bg-muted transition-colors" title="Use in campaign">
                    <Send className="w-3.5 h-3.5 text-muted-foreground" />
                  </button>
                  <button onClick={() => handleDelete(template.id)} className="p-1.5 rounded hover:bg-muted transition-colors" title="Delete">
        ...
        <DialogContent>
          ...
          <div className="space-y-4 pt-2">
            <div>
              ...
              <Input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="e.g. Welcome Message" className="mt-1.5" />
            </div>
            <div>
